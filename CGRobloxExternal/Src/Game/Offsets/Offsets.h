#pragma once
#include <cstdint>
#include <string>

// Roblox Version: version-ddf02245bdbb428c
// Source: https://offsets.imtheo.lol (automatic updater)

namespace offsets {
    inline std::string RobloxVersion = "version-ddf02245bdbb428c";
    inline std::string OffsetSource = "embedded";
    inline bool AutoSyncEnabled = true;
    inline bool LastSyncOk = false;

    inline uintptr_t Adornee = 0xF0;
    inline uintptr_t Anchored = 0x2;
    inline uintptr_t AnchoredMask = 0x2;
    inline uintptr_t AnimationId = 0xC0;
    inline uintptr_t AttributeToNext = 0x58;
    inline uintptr_t AttributeToValue = 0x18;
    inline uintptr_t AutoJumpEnabled = 0x1D4;
    inline uintptr_t BanningEnabled = 0x14C;
    inline uintptr_t BeamBrightness = 0x180;
    inline uintptr_t BeamColor = 0x120;
    inline uintptr_t BeamLightEmission = 0x18C;
    inline uintptr_t BeamLightInfuence = 0x190;
    inline uintptr_t CFrame = 0xC8;
    inline uintptr_t Camera = 0x488;
    inline uintptr_t CameraMaxZoomDistance = 0x1BD;
    inline uintptr_t CameraMinZoomDistance = 0x36C;
    inline uintptr_t CameraMode = 0x370;
    inline uintptr_t CameraPos = 0xFC;
    inline uintptr_t CameraRotation = 0xD8;
    inline uintptr_t CameraSubject = 0xC8;
    inline uintptr_t CameraType = 0x138;
    inline uintptr_t CanCollide = 0x8;
    inline uintptr_t CanCollideMask = 0x8;
    inline uintptr_t CanTouch = 0x10;
    inline uintptr_t CanTouchMask = 0x10;
    inline uintptr_t CharacterAppearanceId = 0x2C8;
    inline uintptr_t Children = 0x70;
    inline uintptr_t ChildrenEnd = 0x8;
    inline uintptr_t ClassDescriptor = 0x18;
    inline uintptr_t ClassDescriptorToClassName = 0x8;
    inline uintptr_t ClickDetectorMaxActivationDistance = 0xE8;
    inline uintptr_t ClockTime = 0x1A8;
    inline uintptr_t CreatorId = 0x180;
    inline uintptr_t DataModelDeleterPointer = 0x0;
    inline uintptr_t DataModelPrimitiveCount = 0x3C0;
    inline uintptr_t DataModelToRenderView1 = 0x1C8;
    inline uintptr_t DataModelToRenderView2 = 0x8;
    inline uintptr_t DataModelToRenderView3 = 0x28;
    inline uintptr_t DecalTexture = 0x180;
    inline uintptr_t Deleter = 0x10;
    inline uintptr_t DeleterBack = 0x18;
    inline uintptr_t Dimensions = 0xAB0;
    inline uintptr_t DisplayName = 0x138;
    inline uintptr_t EvaluateStateMachine = 0x1D8;
    inline uintptr_t FFlagList = 0x0;
    inline uintptr_t FFlagToValueGetSet = 0x30;
    inline uintptr_t FOV = 0x140;
    inline uintptr_t FakeDataModelPointer = 0x7D28508;
    inline uintptr_t FakeDataModelToDataModel = 0x1D0;
    inline uintptr_t FogColor = 0xEC;
    inline uintptr_t FogEnd = 0x124;
    inline uintptr_t FogStart = 0x128;
    inline uintptr_t ForceNewAFKDuration = 0x0;
    inline uintptr_t FramePositionOffsetX = 0x51C;
    inline uintptr_t FramePositionOffsetY = 0x524;
    inline uintptr_t FramePositionX = 0x518;
    inline uintptr_t FramePositionY = 0x520;
    inline uintptr_t FrameRotation = 0x188;
    inline uintptr_t FrameSizeOffsetX = 0x540;
    inline uintptr_t FrameSizeOffsetY = 0x544;
    inline uintptr_t FrameSizeX = 0x538;
    inline uintptr_t FrameSizeY = 0x53C;
    inline uintptr_t FrameVisible = 0x5AD;
    inline uintptr_t GameId = 0x188;
    inline uintptr_t GameLoaded = 0x578;
    inline uintptr_t Gravity = 0x1A4;
    inline uintptr_t Health = 0x188;
    inline uintptr_t HealthDisplayDistance = 0x18C;
    inline uintptr_t HipHeight = 0x194;
    inline uintptr_t HumanoidDisplayName = 0xB8;
    inline uintptr_t HumanoidState = 0x898;
    inline uintptr_t HumanoidStateId = 0x20;
    inline uintptr_t InputObject = 0xF0;
    inline uintptr_t InsetMaxX = 0x100;
    inline uintptr_t InsetMaxY = 0x104;
    inline uintptr_t InsetMinX = 0xF8;
    inline uintptr_t InsetMinY = 0xFC;
    inline uintptr_t InstanceAttributePointer1 = 0x48;
    inline uintptr_t InstanceAttributePointer2 = 0x18;
    inline uintptr_t InstanceCapabilities = 0x208;
    inline uintptr_t JobEnd = 0xD0;
    inline uintptr_t JobId = 0x120;
    inline uintptr_t JobStart = 0xC8;
    inline uintptr_t Job_Name = 0x18;
    inline uintptr_t JobsPointer = 0x82B9F48;
    inline uintptr_t JumpPower = 0x1A4;
    inline uintptr_t JumpHeight = 0x1A0;
    inline uintptr_t LocalPlayer = 0x130;
    inline uintptr_t LocalScriptByteCode = 0x190;
    inline uintptr_t LocalScriptBytecodePointer = 0x10;
    inline uintptr_t LocalScriptHash = 0x1A0;
    inline uintptr_t MaterialType = 0x0;
    inline uintptr_t MaxHealth = 0x1A8;
    inline uintptr_t MaxSlopeAngle = 0x1AC;
    inline uintptr_t MeshPartColor3 = 0x148;
    inline uintptr_t MeshPartTexture = 0x2C0;
    inline uintptr_t ModelInstance = 0x298;
    inline uintptr_t ModuleScriptByteCode = 0x138;
    inline uintptr_t ModuleScriptBytecodePointer = 0x10;
    inline uintptr_t ModuleScriptHash = 0x148;
    inline uintptr_t MoonTextureId = 0xC8;
    inline uintptr_t MousePosition = 0xD4;
    inline uintptr_t MouseSensitivity = 0x0;
    inline uintptr_t MoveDirection = 0x140;
    inline uintptr_t Name = 0x98;
    inline uintptr_t NameDisplayDistance = 0x1B0;
    inline uintptr_t NameSize = 0x10;
    inline uintptr_t OnDemandInstance = 0x0;
    inline uintptr_t OutdoorAmbient = 0xF8;
    inline uintptr_t Parent = 0x68;
    inline uintptr_t PartSize = 0x1B8;
    inline uintptr_t Ping = 0xCC;
    inline uintptr_t PlaceId = 0x190;
    inline uintptr_t PlayerConfigurerPointer = 0x0;
    inline uintptr_t PlayerMouse = 0x11D8;
    inline uintptr_t Position = 0xEC;
    inline uintptr_t Primitive = 0x128;
    inline uintptr_t PrimitiveFlags = 0x1B6;
    inline uintptr_t PrimitiveValidateValue = 0x6;
    inline uintptr_t PrimitivesPointer1 = 0x0;
    inline uintptr_t PrimitivesPointer2 = 0x3C8;
    inline uintptr_t ProximityPromptActionText = 0xB0;
    inline uintptr_t ProximityPromptEnabled = 0x136;
    inline uintptr_t ProximityPromptGamepadKeyCode = 0x11C;
    inline uintptr_t ProximityPromptHoldDuraction = 0x120;
    inline uintptr_t ProximityPromptMaxActivationDistance = 0x128;
    inline uintptr_t ProximityPromptMaxObjectText = 0xD0;
    inline uintptr_t ReadOnlyGravity = 0x998;
    inline uintptr_t RenderJobToDataModel = 0x1C8;
    inline uintptr_t RenderJobToFakeDataModel = 0x38;
    inline uintptr_t RenderJobToRenderView = 0x1D0;
    inline uintptr_t RenderView = 0xBB8;
    inline uintptr_t RequireBypass = 0x0;
    inline uintptr_t RigType = 0x1C0;
    inline uintptr_t RootPartR15 = 0x5D8;
    inline uintptr_t RootPartR6 = 0x478;
    inline uintptr_t Rotation = 0xC8;
    inline uintptr_t RunContext = 0x148;
    inline uintptr_t Sandboxed = 0xC5;
    inline uintptr_t ScreenGuiEnabled = 0x4C4;
    inline uintptr_t ScriptContext = 0x440;
    inline uintptr_t Sit = 0x1DD;
    inline uintptr_t SkyboxBk = 0xF8;
    inline uintptr_t SkyboxDn = 0x128;
    inline uintptr_t SkyboxFt = 0x158;
    inline uintptr_t SkyboxLf = 0x188;
    inline uintptr_t SkyboxRt = 0x1B8;
    inline uintptr_t SkyboxUp = 0x1E8;
    inline uintptr_t SoundId = 0xC8;
    inline uintptr_t StarCount = 0x248;
    inline uintptr_t StringLength = 0x10;
    inline uintptr_t SunTextureId = 0x218;
    inline uintptr_t TagList = 0x0;
    inline uintptr_t TaskSchedulerMaxFPS = 0xB0;
    inline uintptr_t TaskSchedulerPointer = 0x82B9F48;
    inline uintptr_t Team = 0x2D8;
    inline uintptr_t TeamColor = 0x3AC;
    inline uintptr_t TextLabelText = 0xDA0;
    inline uintptr_t TextLabelVisible = 0x5AD;
    inline uintptr_t Tool_Grip_Position = 0x4AC;
    inline uintptr_t Transparency = 0xD0;
    inline uintptr_t UserId = 0x300;
    inline uintptr_t Value = 0xB8;
    inline uintptr_t ValueGetSetToValue = 0xC0;
    inline uintptr_t Velocity = 0xF8;
    inline uintptr_t ViewportSize = 0x2C8;
    inline uintptr_t VisualEngine = 0x10;
    inline uintptr_t VisualEnginePointer = 0x8447C68;
    inline uintptr_t VisualEngineToDataModel1 = 0xA90;
    inline uintptr_t VisualEngineToDataModel2 = 0x1D0;
    inline uintptr_t WalkSpeed = 0x1D0;
    inline uintptr_t WalkSpeedCheck = 0x3BC;
    inline uintptr_t Workspace = 0x160;
    inline uintptr_t WorkspaceToWorld = 0x368;
    inline uintptr_t viewmatrix = 0x150;
}
